    ...
    <div className={`p-4 rounded-xl bg-[#171e30] border transition-all ${isSelected ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-800 hover:border-gray-700'}`}>
      <div className="flex justify-between items-start">
        <div className="flex space-x-4">
          <div className={`w-20 h-20 rounded-lg flex items-center justify-center flex-shrink-0 ${getBgColor(contract.status)}`}>
          ...
          <div className="text-xs space-y-1 text-gray-300">
            ...
            <div className="pt-1 mt-1 border-t border-gray-800/50 space-y-1">
              ...
              <p><span className="text-gray-500 inline-block w-24">Loan Amount</span> : <span>Rs. {Number(contract.LoanAmount || 0).toLocaleString('en-US', {minimumFractionDigits: 2})}</span></p>
              <p><span className="text-gray-500 inline-block w-24">Leasing Amount</span> : <span>Rs. {Number(contract.LeasingAmount || 0).toLocaleString('en-US', {minimumFractionDigits: 2})}</span></p>
            </div>
          </div>
        </div>
